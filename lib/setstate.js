...
proto.executeSetState= function(args,onFinish){
    ...
    if(typeof args === 'object' && args instanceof Object){
        ...
        var mutator= new Mutator(args,this.previousState)
        mutator.finalValue().subscribe(value=>{
                this.newState= value
                this.setComponentState()
        })
    }
...
/*proto.executeOnFinish= async function(){
    if(this.onFinish && typeof this.onFinish === 'function' ){
        if(this.newState) this.onFinish(this.previousState,this.currentState)
        else this.onFinish(this.previousState)
    }
}*/
